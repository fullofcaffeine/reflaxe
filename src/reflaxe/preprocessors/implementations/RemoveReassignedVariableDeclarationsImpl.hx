	...
	public function removeUnnecessaryVarDecls():Array<TypedExpr> {
		...
		while (i < exprList.length) {
			...
			switch (expr.expr) {
				case TVar(tvar, e):
					{
						// A previous local can be read while initializing this one. Inspect
						// that value before considering the new declaration removable.
						if (e != null) {
				...
				case TBlock(el):
					{
						// Reads in a nested lexical block still make an earlier
						// declaration observable from this enclosing sequence.
